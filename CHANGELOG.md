...
Фикс бага в `usePaginate()`: подписка на кэш не переподключалась при
смене `params`, если номер/набор загруженных страниц не менялся
(типовий кейс — первое переключение фильтра). Компонент навсегда
зависал в `isLoading: true`, хотя данные уже были в кэше. Тот же баг
проявлялся при явном `reset()`.

### Fixed
- `useEffect` подписки на кэш (`src/hooks/use-paginate.ts`) теперь
  дополнительно зависит от хэша `keyPrefix` (`params`/`customKey`), а
  не только от набора номеров страниц — переподключается корректно
  при смене `params`.

### Tests
- +2 регрессионных теста: смена `params` при неизменном наборе
  подписанных страниц, в обоих режимах (`page` и `infinite`).
