...
## Додаток (2.2.2): той самий клас бага в reset()

Після випуску 2.2.1 користувач повідомив про той самий симптом
("зайшов у деталь, повернувся у список — вічне завантаження") —
типовий сценарій `useFocusEffect(() => reset())` при поверненні на
екран списку. 2.2.1 виправляє лише РЕЗОНАНС на зміну `params`
(додано `keyPrefixHash` у залежності ефекту підписки) — але `reset()`
не змінює `params` і не обов'язково змінює номер сторінки (якщо
компонент і так стояв на `initialPage`), тож `subscribedPagesKey` і
`keyPrefixHash` можуть лишитись ОБИДВА незмінними навколо виклику
`reset()`.

Корінь той самий, ще на рівень глибше: `reset()`
(`src/hooks/use-paginate.ts:360-373`) викликав `cache.remove()`, який
**видаляє запис із `Map`**. Наступний `cache.fetch()` через
`ensureEntry()` створює **новий об'єкт запису** з порожньою
`subscribers`. Підписка (`cache.subscribe()`) прив'язана до
КОНКРЕТНОГО об'єкта запису, захопленого в замиканні на момент
виклику — вона не перерозв'язується за ключем при кожному
використанні. Отже, ЛЮБА заміна об'єкта запису під тим самим ключем
осиротжує підписку, незалежно від того, чи змінився сам ключ.

**Фікс:** `reset()` замінено з `cache.remove(predicate)` на
`cache.invalidate(predicate)` (`src/query/cache.ts:252-267`) —
`invalidate()` мутує ІСНУЮЧИЙ об'єкт запису на місці (`isStale: true`
+ `notify()` наявних підписників), ніколи не видаляючи його з `Map`.
Наступний `cache.fetch()` знаходить той самий об'єкт через
`ensureEntry()` (а не створює новий), бо `isStale: true` вже сама
собою вимикає гейт свіжості (`isFresh` стає `false`) — рефетч
відбувається так само, без потреби в `force: true`.

**Тест** (`src/__tests__/use-paginate-infinite.test.tsx`, `'reset()
СТОЯЧИ на першій сторінці (без попереднього fetchNextPage) не орфанує
підписку'`) — навмисно НЕ викликає `fetchNextPage()` перед `reset()`
(на відміну від сусіднього тесту `'reset() в infinite очищает
аккумулятор'`, який випадково маскує баг, бо `fetchNextPage()` перед
`reset()` змінює `subscribedPagesKey` з `"1,2"` на `"1"`). Перевіряє
через `_debugEntries()`, що запис ДО і ПІСЛЯ `reset()` — той самий
об'єкт (`toBe`), з тим самим підписником. RED на 2.2.1 (порожня
`subscribers` після reset), GREEN після фіксу.

Версія: `2.2.1` → `2.2.2` (ще один patch — той самий клас бага, інший
код-шлях).
